...
v_form($_POST[db_adicionar],"Campo Oculto","?acao=banner&subacao=adicionar");
v_form($_POST[banner_url],"URL do banner","?acao=banner&subacao=adicionar");
v_form($_POST[banner_link],"Link do site do anunciante","?acao=banner&subacao=adicionar");
v_form($_POST[banner_tipo],"Tipo do banner","?acao=banner&subacao=adicionar");
v_form($_POST[banner_tamanho],"Tamanho do banner","?acao=banner&subacao=adicionar");
v_form($_POST[banner_zona],"Zona onde o banner irá aparecer","?acao=banner&subacao=adicionar");
v_form($_POST[banner_limitacao],"Limitações do banner","?acao=banner&subacao=adicionar");
v_form($_POST[dia],"Campo Oculto","?acao=banner&subacao=adicionar");
v_form($_POST[mes],"Campo Oculto","?acao=banner&subacao=adicionar");
v_form($_POST[ano],"Campo Oculto","?acao=banner&subacao=adicionar");
v_form($_POST[banner_anunciante],"Anunciante","?acao=banner&subacao=adicionar");

if ($_POST[banner_limitacao]=="clicks") {
v_form($_POST[banner_q_clicks],"Quantidade de Clicks","?acao=banner&subacao=adicionar");
$q_limitacao=$_POST[banner_q_clicks];
}
if ($_POST[banner_limitacao]=="views") {
v_form($_POST[banner_q_views],"Quantidade de Views","?acao=banner&subacao=adicionar");
...
$msg_erro="Você não pode adicionar esse banner a esta zona porque esta<br>
    ...
    Redirecionando se não quiser esperar <a href=?acao=banner&subacao=adicionar>clique aqui</a>
    <meta http-equiv=\"refresh\" content=\"3;URL=?acao=banner&subacao=adicionar\">";
    include "msg_erro.php";
    exit; }

$sel_url_r=mysql_query("SELECT url FROM banners WHERE url='$_POST[banner_url]' and user_id='$_SESSION[user_id]'",$link_db);
$r_url=mysql_num_rows($sel_url_r);
if ($r_url != "0") {
$msg_erro="Este banner já está cadastrado no sistema.<br>
    Redirecionando se não quiser esperar <a href=?acao=banner&subacao=adicionar>clique aqui</a>
    <meta http-equiv=\"refresh\" content=\"3;URL=?acao=banner&subacao=adicionar\">";
    include "msg_erro.php";
    exit; }
...
$data_c=date("d/m/Y");
    $cad_banner=mysql_query("INSERT INTO banners VALUES ('','$_SESSION[user_id]','$_POST[banner_anunciante]','$_POST[banner_link]','$_POST[banner_url]','$_POST[banner_tipo]','$largura','$altura','0','0','$data_c','$_POST[banner_ativo]','$_POST[banner_zona]','$_POST[banner_limitacao]','$q_limitacao')",$link_db);
if ($l_zona[0] != $_POST[banner_tamanho]) {
$msg_erro="<center>Banner cadastrado com sucesso<br>
    Redirecionando se não quiser esperar <a href=?action=ver_banners&z_id=$l_zona[0]>clique aqui</a>
    <meta http-equiv=\"refresh\" content=\"10;URL=?action=ver_banners&z_id=$l_zona[0]\">
</center>";
...
$r_zonas=mysql_query("SELECT id from zonas where user_id='$_SESSION[user_id]'",$link_db);
$n_zonas=mysql_num_rows($r_zonas);
$r_anunciantes=mysql_query("SELECT id from anunciantes where user_id='$_SESSION[user_id]'",$link_db);
$n_anunciantes=mysql_num_rows($r_anunciantes);

if ($n_zonas=="0" || $n_anunciantes=="0") {
if ($n_zonas=="0") {
$msg="Nenhuma zona cadastrada, você deve adicionar zonas antes de adicionar um banner.<br>
Cadastre uma zona clicando em Nova Zona no menu acima.";
}
if ($n_anunciantes=="0") {
$msg="Nenhum anunciante cadastrado, você deve adicionar anunciantes antes de adicionar um banner.<br>
Cadastre uma anunciante clicando em Novo Anunciante no menu acima.";
}
?>
...
<? } else { ?>
<form method="post" action="?acao=banner&subacao=adicionar">
<input type="hidden" name="db_adicionar" value="ok" />
<table border="0" align="center" width="500" cellspacing="2" cellpadding="0">
<tr>
<td colspan="2" align="center" class="cel_menu"><b>Adicionar Banner</b></td>
</tr>
...
<td><input type="text" value="http://" size="60" name="banner_url" class="form_txt" OnFocus="this.className='form_txt_hover';" OnBlur="this.className='form_txt';" /></td>
</tr>
<tr>
<td>Link do Site Anunciante:</td>
<td><input type="text" size="60" value="http://" name="banner_link" class="form_txt" OnFocus="this.className='form_txt_hover';" OnBlur="this.className='form_txt';" /></td>
</tr>
<tr>
...
<option value="flash">Flash (.swf)</option>
...
<tr>
<td rowspan="2" align="center" class="cel_banner"><input type="radio" checked name="banner_limitacao" value="N" /> Ilimitado</td>
...
<?
$r_zona=mysql_query("SELECT nome FROM anunciantes WHERE user_id='$_SESSION[user_id]'",$link_db);
while ($zona=mysql_fetch_row($r_zona)) {
?>
...
<tr>
<td></td>
<td align="center"><input type="submit" value="Cadastrar Banner" class="form_bt" OnFocus="this.className='form_bt_hover';" OnBlur="this.className='form_bt';"/></td>
</tr>
</table>
...
v_form($_POST[banner_url],"URL do banner","?");
v_form($_POST[banner_link],"Link do site do anunciante","?");
v_form($_POST[banner_tipo],"Tipo do banner","?");
v_form($_POST[banner_tamanho],"Tamanho do banner","?");
v_form($_POST[banner_zona],"Zona onde o banner irá aparecer","?");
v_form($_POST[banner_limitacao],"Limitações do banner","?");
v_form($_POST[dia],"Campo Oculto","?");
v_form($_POST[mes],"Campo Oculto","?");
v_form($_POST[ano],"Campo Oculto","?");
v_form($_POST[banner_anunciante],"Anunciante","?");
v_form($_POST[banner_id],"Campo Oculto","?");

if ($_POST[banner_limitacao]=="clicks") {
v_form($_POST[banner_q_clicks],"Quantidade de Clicks","?");
$q_limitacao=$_POST[banner_q_clicks];
}
if ($_POST[banner_limitacao]=="views") {
v_form($_POST[banner_q_views],"Quantidade de Views","?");
$q_limitacao=$_POST[banner_q_views];
}
if ($_POST[banner_limitacao]=="data") {
$q_limitacao=$_POST[ano].$_POST[mes].$_POST[dia];
...
$msg_erro="Banner Editado com sucesso<br>
    Redirecionando se não quiser esperar <a href=?action=ver_banners&z_id=$l_zona[0]>clique aqui</a>
    <meta http-equiv=\"refresh\" content=\"10;URL=?action=ver_banners&z_id=$l_zona[0]\"><br><br>";
...
?>
<form method="post" action="?acao=banner&subacao=editar">
...
v_form($_POST[b_id],"Campo Oculto","?");
...
$msg_erro="Nenhum banner selecionada.<br>
    ...
    exit; }
v_form($_GET[b_id],"Campo Oculto","?");
$sel_banner=mysql_query("SELECT id FROM banners WHERE id='$_GET[b_id]'",$link_db);
$banner=mysql_fetch_array($sel_banner);
?>
<form method="post" action="?acao=banner&subacao=deletar">
<input type="hidden" value="ok" name="db_deletar" />
<input type="hidden" value="<?=$banner[id]?>" name="b_id" />
<table border="0" cellspacing="2" cellpadding="0" align="center">
<tr>
...
<? } } elseif ($_GET[subacao]=="ativar") {
if (!$_GET[b_id]) {
$msg_erro="Nenhuma banner selecionada.<br>
    Redirecionando se não quiser esperar <a href=?>clique aqui</a>
    <meta http-equiv=\"refresh\" content=\"3;URL=?\">";
    include "msg_erro.php";
    exit; }
v_form($_GET[b_id],"Campo Oculto","?");
$ativar=mysql_query("UPDATE banners SET ativo='1'  WHERE id='$_GET[b_id]'",$link_db) or die (mysql_error());
if ($ativar) {
$msg_erro="Banner Ativado com sucesso<br>
    Redirecionando se não quiser esperar <a href=?>clique aqui</a>
    <meta http-equiv=\"refresh\" content=\"10;URL=?\">";
    include "msg_erro.php";
    exit; }
} elseif ($_GET[subacao]=="desativar") {
if (!$_GET[b_id]) {
$msg_erro="Nenhuma banner selecionada.<br>
    Redirecionando se não quiser esperar <a href=?>clique aqui</a>
    <meta http-equiv=\"refresh\" content=\"3;URL=?\">";
    include "msg_erro.php";
    exit; }
v_form($_GET[b_id],"Campo Oculto","?acao=banner&subacao=editar");
$desativar=mysql_query("UPDATE banners SET ativo='0'  WHERE id='$_GET[b_id]'",$link_db) or die (mysql_error());
if ($desativar) {
$msg_erro="Banner Desativado com sucesso<br>
    Redirecionando se não quiser esperar <a href=?>clique aqui</a>
    <meta http-equiv=\"refresh\" content=\"10;URL=?\"> ";
     include "msg_erro.php";
    exit; }
